$(function(){
	...
	$(".more").on("click", function(){
		...
		if($(this).hasClass("moreOn")){
			$(".portfolio_detail .portfolio_left").css({bottom:0});
			$(this).text("BACK");
		}else{
			$(".portfolio_detail .portfolio_left").css({bottom:"-100vh"});
...
function listFn(){
	$.ajax({
		...
		var imgUrl='', concept='', color='', colorTag ='', colorSP='', imgSP = '', imgTag = '', imgW = '', skill='', tit='', idx = 0, tag = '', j=0, detailurl='', detailurltxt='';
		pageUrl = location.href;
		pageSP = pageUrl.split('?');
		page = parseInt(pageSP[1]);
			
		function dataLoad(){
			itemNum = 0, imgUrl='', concept='', color='', colorTag ='',colorSP='',imgSP = '', imgTag = '', imgW = '', skill='', tit='', idx = 0, tag = '', j=0 , k='', detailurl='', detailurltxt='';
			switch(page){
				case 0:pageName = "Device"; break; //UI UX 디바이스
				case 1:pageName = "Web"; break; //UI UX 웹
				case 2:pageName = "Graphic"; break; //그래픽 프로모션
			}
		...
		$(".portfolio_card .btn_tab li").on("click", function(){
			$(".portfolio_all").css({display:"none"});
			$(".portfolio_all").eq($(this).index()).css({
				display:"flex"
			});
			page = $(this).index();
		...
		function dbdata(){
			...
			$(".portfolio_all").eq(page).find(".portfolio_detailright ul").stop().delay(200).animate({
				...
				for(var i=0; i<imgSP.length; i++){
					imgTag += "<li><img src='"+imgSP[i]+"' alt=''/></li>";
				}
			...
			$(".portfolio_all").eq(page).find(".left_detail").stop().animate({
				...
				detailurl = $(data).find("item").eq(itemNum).find("durl").text();
				detailurltxt = $(data).find("item").eq(itemNum).find("durltxt").text();
				...
				var hh = $(".portfolio_index span").eq(idx).text(),
				h = ''; if(j<9){h = '0';}else{h=''}

				//bottom index
				$(".all_index").find("code:first").text(hh);
				$(".all_index").find("code:nth-of-type(2)").text(h+($(".portfolio_index span").length));
				
				//portfolio detail
				$(".portfolio_all").eq(page).find(".left_detail h4").text(tit+" "+hh);
				$(".portfolio_all").eq(page).find(".detailurl a").attr("href",detailurl);
				$(".portfolio_all").eq(page).find(".detailurl a").text(detailurltxt);
